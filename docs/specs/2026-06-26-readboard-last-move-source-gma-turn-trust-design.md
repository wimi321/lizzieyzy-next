...
2. `snapshotDelta.hasMarker()` 且 source 是 `redBlueMarker` 或 `foxCornerFlip`：按 marker 棋色推断下一手。
3. source 是 `deviation` 或 `stoneCount`：marker 不能作为 GMA 权威轮次。
4. markerless 普通局才允许 `foxMoveNumber % 2` 兜底。
5. 其他情况回到现有 `inferBlackToPlayWithoutMarker(...)` 的保守路径。
