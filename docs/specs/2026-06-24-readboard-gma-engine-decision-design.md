...
- `time > 0`：首次覆盖前用 `kata-get-param maxTime` 保存原始运行时值，然后在本手前发送 `kata-set-param maxTime <time>`。
- `playouts > 0`：首次覆盖前用 `kata-get-param maxVisits` 保存原始运行时值，然后在本手前发送 `kata-set-param maxVisits <playouts>`，以保持现有“最大计算量”依据 `visits` 的语义。
- 空白和 `0` 均不设置对应正数限制；如果本 GMA 会话此前覆盖过该参数，则先恢复首次读取到的原始运行时值，再发起下一手。两个都为空或 `0` 时完全交给 KataGo 配置中的 `maxTime` / `maxVisits`。
- 停止同步、退出 GMA 模式或清理 GMA 自动落子状态时，也要恢复此前覆盖过的 `maxTime` / `maxVisits`。如果停止发生得很早、原始值还在 `kata-get-param` 响应途中，则记录一次延迟恢复；原始值返回后补发 restore。若原始值返回前又开始新的正数 GMA，则取消这次延迟恢复，继续复用首次读取请求，避免把新一轮搜索刚设置的限制恢复掉。
...
`scheduleReadBoardGmaIfNeeded(...)` 只在已同步局面的 side-to-play 可信时启动 GMA。可信来源包括真实视觉 marker、空盘默认黑方开局、ReadBoard“交换顺序”产生的显式手动覆盖、同步主流程已接受并落地的真实单步 `MOVE`、Fox `foxMoveNumber 0` 且全黑多子 setup 的让子初始局默认白方，以及无 setup / handicap 风险的 markerless ordinary `foxMoveNumber` fallback；不可信时跳过本次调度并保留自动落子配置，等待下一帧可信局面再判断。
...
- 不把 ReadBoard“交换顺序”的 `pass` 行解释成真实 `PASS`；它只用于显式切换当前轮次并重新调度 GMA。
...
3. `time` / `playouts` 的原始 `0` 与正数通过 `kata-set-param` 正确设置或恢复原始值，`firstPolicy` 不影响 `gma` 最终选择。
